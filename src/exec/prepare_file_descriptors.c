...
/*   Updated: 2025/02/07 13:39:00 by smoore           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_write_files(t_outs *outs, t_data *data)
{
	...
	{
		if (ptr->trunc_fn)	
			data->write_fd = open(ptr->trunc_fn, O_CREAT | O_WRONLY | O_TRUNC, 0644);
		else if (ptr->append_fn)	
			data->write_fd = open(ptr->append_fn,  O_CREAT | O_WRONLY | O_APPEND, 0644);
		ptr = ptr->next;
	}
	if (data->write_fd == -1)
		return ;
